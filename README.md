...
Esta es una **versión extendida** del gateway **Arcanum** original. El **motor** —la parte
verdaderamente difícil: la firma **WSAA en CMS/PKCS#7 100% local**, el **caché de tokens**,
el **catálogo de servicios SOAP** y la traducción **REST ⇄ SOAP**— es obra del **proyecto
Arcanum original** (ver [Atribución y licencia](#atribución-y-licencia)). Esta versión **se
inspira en ese trabajo y lo conserva en su núcleo**, y agrega encima una **capa completa de
facturación masiva** pensada para dos realidades concretas: asociaciones médicas que liquidan
a sus profesionales, y PyMEs que facturan a sus clientes.

**Qué agrega esta versión sobre el motor base:**

- **Panel de emisión masiva con dos perfiles** (Asociación médica / PyME): cambian los
  formularios, las columnas y los flujos según el caso de uso.
- **Importación de lotes desde Excel** (un comprobante "global" por obra social, con una
  fila por profesional), **acumulativa** y con **detección de duplicados** por comprobante global.
- **Representados**: emisión a terceros con tu propio certificado (delegación de ARCA), con
  alta/baja/edición por perfil e importación desde `.xlsx`.
- **Seguimiento de pendientes**: a quién le pediste factura y todavía no cargaste, con
  **semáforo por antigüedad**, **reenvío** de la solicitud por email y marcado de "cargada".
- **Importación de comprobantes ya emitidos** leyendo su **PDF** (QR de ARCA con respaldo por
  texto), con alertas de **CAE duplicado** y de **CUIT de emisor que no corresponde**.
- **Envío de facturas por email** con el PDF adjunto, y **exportación a CSV diferenciada** por
  perfil (pensada también para el Libro IVA).
- **Datos del emisor por variable de entorno** (`ARCANUM_EMISORES`), para no exponer datos
  reales en el repositorio.

La guía de uso de toda esta capa está en **[FACTURADOR.md](FACTURADOR.md)**.

> Lo que sigue documenta el **motor base** (heredado del proyecto original), que esta versión
> conserva en su núcleo. Para los detalles del facturador, ver FACTURADOR.md.
...
> **Facturación en lote.** Sobre esta base hay un panel de **emisión masiva** con dos
> perfiles (asociación médica que liquida a profesionales, y PyME que factura a clientes),
> representados, seguimiento de pendientes e importación de comprobantes ya emitidos.
> Ver **[FACTURADOR.md](FACTURADOR.md)**.
...
Este proyecto es una **obra derivada** del gateway **Arcanum**, creado originalmente por
**Diego Alejandro Parras** y
publicado bajo **Apache License 2.0**.

- Se conserva la **licencia original** (ver [LICENSE](LICENSE)) y este aviso de atribución,
  tal como lo exige la Apache-2.0.
- Esta versión **modifica y extiende** el proyecto original (ver
  [Sobre esta versión](#sobre-esta-versión) y [CHANGELOG.md](CHANGELOG.md)); los archivos
  modificados pueden diferir del original.
- **Proyecto original:** Arcanum, de Diego Alejandro Parras.
- Ver también el archivo [NOTICE](NOTICE).

El código de esta versión se distribuye igualmente bajo **Apache License 2.0**. Arcanum no
está afiliado ni respaldado por ARCA/AFIP.
