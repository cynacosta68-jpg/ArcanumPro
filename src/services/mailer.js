...
async function send(to, subject, text, attachments) {
  ...
  const parrafos = String(text || '').split(/\n/).map((l) => l.trim()).filter(Boolean).map((l) => `<p style="margin:0 0 16px 0;line-height:2">${esc(l)}</p>`).join('');
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#222">${parrafos}</div>`;
